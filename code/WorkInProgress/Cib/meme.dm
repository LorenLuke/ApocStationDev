...
// Needs work to work with gamemeodes
datum/meme
...
mob/living/parasite/meme
	var/mob/captive_mind/meme_captive_mind= new()
...
mob/living/parasite/meme/Life()
	...
	if(host.reagents.has_reagent("stoxin"))
		src.mind << "\red <b>Something in your host's blood makes you lose consciousness... you fade away...</b>"
		src.death()
		return
	// a host without brain is no good
	if(!host.mind)
		spawn (20)
			src.mind << "\red <b>Your host has no mind... you fade away...</b>"
			src.death()
		return
	if(host.stat == 2)
		spawn (20)
			src.mind << "\red <b>Your host has died... you fade away...</b>"
			src.death()
		return
...
mob/living/parasite/meme/say(message as text)
	if(dormant)
		...
		return
	if(!host)
		usr << "\red You can't speak without a host!"
		return
...
mob/living/parasite/meme/me_verb(message as text)
	...
	if(!host)
		usr << "\red You can't emote without a host!"
		return
...
mob/living/parasite/meme/proc/select_indoctrinated(var/title, var/message)
	...
	// Can only affect other mobs than the host if not blinded
	if(blinded)
		candidates = list()
...
mob/living/parasite/meme/verb/Thought()
	set category = "Meme"
	set name	 = "Thought(50)"
	set desc     = "Implants a thought into an attuned mind, making them think they heard someone talk."
	...
	var/mob/living/carbon/human/target = select_indoctrinated("Thought", "Select a target which will hear your thought.")
...
mob/living/parasite/meme/verb/Mute()
	set category = "Meme"
	set name	 = "Mute(250)"
	set desc     = "Prevents an attuned mind from talking for a while."

	var/mob/living/carbon/human/target = select_indoctrinated("Mute", "Select a target which will be te temporarily rendered mute.")

	if(!target) return
	if(target.silent > 0)
		usr << "\red Your target already can't speak..."
		return
	...
	spawn
		...
		target << "\red Your tongue feels numb. You lose your ability to speak."
		usr << "\red The target can no longer speak."

		target.silent += 120

		sleep(1200)

		target<< "\red Your tongue has feeling again."
		usr << "\red [target] can speak again."

// Makes the host unable to emote
mob/living/parasite/meme/verb/Paralyze()
	set category = "Meme"
	set name	 = "Paralyze(250)"
	set desc     = "Prevents an attuned mind from using emote for a while."

	var/mob/living/carbon/human/target = select_indoctrinated("Paralyze", "Select a target which will be temporarily unable to emote.")
	...
	spawn
		...
		target << "\red Your body feels numb. You lose your ability to use body language."
		...
		target.use_me = 1
		target << "\red Your body has feeling again."
		usr << "\red [target] can use body language again."

// Cause great agony with the host, used for conditioning the host
mob/living/parasite/meme/verb/Agony()
	set category = "Meme"
	set name	 = "Agony(200)"
	set desc     = "Causes significant pain in an attuned mind."

	var/mob/living/carbon/human/target = select_indoctrinated("Agony", "Who should experience agony?")
	...
	spawn
		...
		target << "\red <font size=5>You feel excrutiating pain all over your body! It is so bad you can't think or articulate yourself properly...</font>"

		usr << "<b>You send a jolt of agonizing pain through [target], they should be unable to concentrate on anything else for half a minute.</b>"
		...
		for(var/i=0, i<10, i++)
			target.stuttering = 2
			sleep(50)
			if(prob(80)) target.flash_weak_pain()
			if(prob(10)) target.paralysis = max(target.paralysis, 2)
			if(prob(15)) target.emote("twitch")
			else if(prob(15)) target.emote("scream")
			else if(prob(10)) target.emote("collapse")

			if(i == 10)
				target << "\red THE PAIN! AGHH, THE PAIN! MAKE IT STOP! ANYTHING TO MAKE IT STOP!"

		target << "\red The pain subsides..."

// Cause great joy with the host, used for conditioning the host
mob/living/parasite/meme/verb/Joy()
	set category = "Meme"
	set name	 = "Joy(200)"
	set desc     = "Causes significant joy in an attuned mind."

	var/mob/living/carbon/human/target = select_indoctrinated("Joy", "Who should experience joy?")

	if(!target) return
	if(!use_points(200)) return

	spawn
		target.druggy = max(target.druggy, 50)
		target.slurring = max(target.slurring, 10)

		usr << "<b>You stimulate [target.name]'s brain, injecting waves of endorphines and dopamine into the tissue. They should now forget all their worries, particularly relating to you, for around a minute."

		target<< "\red You are feeling wonderful! Your head is numb and drowsy, and you can't help forgetting all the worries in the world."

		while(target.druggy > 0)
			sleep(10)

		target << "\red You are feeling clear-headed again..."

// Cause the target to hallucinate.
mob/living/parasite/meme/verb/Hallucinate()
	set category = "Meme"
	set name	 = "Hallucinate(300)"
	set desc     = "Makes an attuned mind hallucinate, has a short delay."
...
//
mob/living/parasite/meme/verb/SubtleJump(mob/living/carbon/human/target as mob in view(1, host))
	set category = "Meme"
	...
	if (host.silent || (host.disabilities & 64) || istype(wear_mask, /obj/item/clothing/mask/muzzle) || (halloss>43))
	...
	if(target.parasites.len > 0)
		if(target == host)
			src << "<b>You're already inhabiting this target.</b>"
			return
...
mob/living/parasite/meme/verb/ObviousJump(mob/living/carbon/human/target as mob in hearers(7, host))
	set category = "Meme"
...
mob/living/parasite/meme/verb/AttunedJump(mob/living/carbon/human/target as mob in indoctrinated)
	set category = "Meme"
...
mob/living/parasite/meme/verb/Attune()
	...
	usr << "<b>You successfully attune [host]."
...
/*
mob/living/parasite/meme/proc/AddMemeVerbs(var/mob/living/carbon/human/host)
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Thought()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Mute()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Paralyze()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Agony()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Joy()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Hallucinate()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/SubtleJump()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/ObviousJump()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/AttunedJump()
	host.verbs += /mob/living/parasite/meme/memeverb/proc/Analgesic()
*/
...
//		host.AddMemeVerbs()
		...
		meme_captive_mind << "\blue You feel very drowsy. Your eyelids become heavy..."
		...
		for(var/i = 1, i<600, i)
			if(!meme_mind)
				host << "\red <b>Your host has died... you fade away...</b>"
				meme_captive_mind << "\red <b>You feel as if you're floating for an instant before fading away...</b>"
				meme_mind.transfer_to(src)
				meme_captive_mind.mind.transfer_to(host)
				src.death()
				i = 600
				world << "MEME DEATH!"
			else if(i > 600)
				world << "MEME POSSESSION ENDED!"
			sleep(10)
			i += 10


//				Depossession(meme_mind, host_mind)

//		if(host.mind == meme_mind)
//			Depossession(meme_mind, host_mind)

/*
mob/living/parasite/meme/verb/Depossession()
	set category = "Meme"
	set name	 = "Depossession"
	set desc     = "Release control of the host."
...
//Verb block for depossess
host = src

var/list/parasite_list = list()
for (mob/living/parasite/meme/M in host.parasites)
	var/list/parasite_list += M
	var/mob/living/parasite/meme = parasite_list[1]



*/
//mob/living/parasite/meme/verb/Depossession(var/datum/mind/meme_mind, var/datum/mind/host_mind)
//	set category = "Meme"
//	set name	 = "Depossession(0)"
//	set desc     = "Release direct control of the host."
...
mob/living/parasite/meme/verb/Dormant()
	...
	usr << "<b>You enter a state of dormancy. You won't be able to take action until all your points have recharged.</b>"
	...
	usr << "\red You awaken from your dormant state, your consciousness coherent and refreshed!"
...
/mob/living/parasite/meme/Stat()
	...
	if (client && client.statpanel == "Status")
		stat(null, "Meme Points: [src.meme_points]/[MAXIMUM_MEME_POINTS]")
