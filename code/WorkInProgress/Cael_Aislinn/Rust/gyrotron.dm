...
/obj/machinery/rust/gyrotron
	...
	Topic(href, href_list)
		...
		if( href_list["modifypower"] )
			var/new_val = text2num(input("Enter new emission power level (0.001 - 0.1)", "Modifying power level (MeV)", mega_energy))
			if(!new_val)
				usr << "\red That's not a valid number."
				return
			new_val = min(new_val,0.1)
...
/obj/machinery/rust/gyrotron/control_panel
	...
	name = "Gyrotron Control panel"
	var/obj/machinery/rust/gyrotron/owned_gyrotron
	var/list/gyros = new()
	New()
		..()
		for(var/obj/machinery/power/gyrotron in range(scan_range, src))
			gyros += gyrotron
		owned_gyrotron = gyros[1]
	...
	interact(mob/user)
		...
		if(owned_gyrotron && owned_gyrotron.on)
			...
			t += "<A href='?src=\ref[src];locate=1'>Close</A><BR>"
...
/obj/machinery/rust/gyrotron/control_panel/attack_hand(mob/user)
	add_fingerprint(user)
	interact(user)

/obj/machinery/rust/gyrotron/attackby(obj/item/W, mob/user)
