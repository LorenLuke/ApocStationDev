...
datum/mind
	...
	var/datum/meme/meme		//meme holder
...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
	...
	Topic(href, href_list)
		...
		if (href_list["role_edit"])
			...
			var/new_obj_type = input("Select objective type:", "Objective type", def_value) as null|anything in list("assassinate", "debrain", "protect", "prevent", "harm", "brig", "hijack", "escape", "survive", "steal", "download", "nuclear", "capture", "absorb", "attune", "custom")
			...
			switch (new_obj_type)
				...
				if("download","capture","absorb","attune")
					var/def_num
