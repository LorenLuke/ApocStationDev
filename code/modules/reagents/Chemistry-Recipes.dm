		...
		slimefreeze
			...
			on_reaction(var/datum/reagents/holder)
				...
				spawn(10)
					playsound(get_turf_loc(holder.my_atom), 'sound/effects/phasein.ogg', 100, 1)
					var/cold = 0
					for(var/mob/living/M in range (get_turf_loc(holder.my_atom), 1))
						M.bodytemperature -= 70
						cold ++
					for(var/mob/living/M in range (get_turf_loc(holder.my_atom), 2))
						M.bodytemperature -= 60
						cold ++
					for(var/mob/living/M in range (get_turf_loc(holder.my_atom), 3))
						M.bodytemperature -= 50
						cold ++
					for(var/mob/living/M in range (get_turf_loc(holder.my_atom), 5))
					...
					for(var/mob/living/M in range (get_turf_loc(holder.my_atom), 7))
						...
						cold ++
						switch (cold)
							if(1) M << "\blue You feel a chill."
							if(2) M << "\blue You feel a strong chill!"
							if(3) M << "\blue You feel a blast of sharp cold!"
							if(4) M << "\blue You feel a strong blast of extreme cold!"
							if(5) M << "\blue <b>You feel like your entire body is ice</b>!"
					world << "[cold]"
