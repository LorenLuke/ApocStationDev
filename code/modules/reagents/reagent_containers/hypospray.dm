...
/obj/item/weapon/reagent_containers/hypospray/autoinjector/verb/empty()

	set name = "Empty Autoinjector"
	set category = "Object"
	set src in usr

	if (alert(usr, "Are you sure you want to empty that?", "Empty Autoinjector:", "Yes", "No") != "Yes")
		return
	if(isturf(usr.loc))
		usr << "<span class='notice'>You empty \the [src] reagents vial onto the floor.</span>"
