...
/obj/machinery/atm/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/weapon/card))
		...
		if(!held_card)
			...
			idcard.loc = src
			held_card = idcard
			if(authenticated_account && ((held_card.associated_account_number != authenticated_account.account_number)||!(authenticated_account.account_number in held_card.secondary_account_numbers)))
				authenticated_account = null
		...
		switch(href_list["choice"])
			...
			if("attempt_auth")
				...
				if(!ticks_left_locked_down && held_card)
					...
					authenticated_account = attempt_account_access(tried_account_num, tried_pin, held_card && ((held_card.associated_account_number == tried_account_num)||(tried_account_num in held_card.secondary_account_numbers)) ? 2 : 1)
					if(!authenticated_account)
