...
/obj/item/device/eftpos/proc/scan_card(var/obj/item/weapon/card/I)
	if (istype(I, /obj/item/weapon/card/id))
		...
		if(transaction_locked && !transaction_paid)
			if(linked_account)
				if(!linked_account.suspended)
					var/list/payer_accounts = list(C.associated_account_number)+C.secondary_account_numbers
					var/list/payer_accounts_name = new()
					for(var/i=1, i<=payer_accounts.len, i++)
						var/account_number = payer_accounts[i]
						var/datum/money_account/account_proper = get_account(account_number)
						payer_accounts_name += account_proper.owner_name
					var/choice = input(usr,"Select Account to use", "Confirm", usr) in payer_accounts_name
					var/attempt_pin = input("Enter pin code", "EFTPOS transaction") as num
					var/account_index = payer_accounts_name.Find(choice)
					var/selected_account_number = payer_accounts[account_index]
					var/datum/money_account/D = attempt_account_access(selected_account_number, attempt_pin, 2)
					if(D)
						if(!D.suspended)
							if(transaction_amount <= D.money)
								usr << "\icon[src] Transaction complete!"
								playsound(src, 'sound/machines/chime.ogg', 50, 1)
								usr.visible_message("\icon[src] The [src] chimes.")
								transaction_paid = 1
								...
								usr.visible_message("\icon[src] The [src] flashes 'Not enough funds!'")
						else
							usr << "\icon[src]<span class='warning'>Your account has been suspended.</span>"
							playsound(src.loc, 'sound/machines/buzz-sigh.ogg', 50, 0)
							usr.visible_message("\icon[src] The [src] flashes 'Input account suspended!'")
					else
						usr << "\icon[src]<span class='warning'>Unable to access account. Check security settings and try again.</span>"
						playsound(src.loc, 'sound/machines/buzz-sigh.ogg', 50, 0)
						usr.visible_message("\icon[src] The [src] flashes 'Cannot access account!'")
				else
					usr << "\icon[src]<span class='warning'>Connected account has been suspended.</span>"
					playsound(src.loc, 'sound/machines/buzz-sigh.ogg', 50, 0)
					usr.visible_message("\icon[src] The [src] flashes 'Recipient account suspended!'")
			else
				usr << "\icon[src]<span class='warning'>EFTPOS is not connected to an account.</span>"
				playsound(src.loc, 'sound/machines/buzz-sigh.ogg', 50, 0)
				usr.visible_message("\icon[src] The [src] flashes 'Not connected to account!'")
	else if (istype(I, /obj/item/weapon/card/emag))
		if(transaction_locked)
			if(transaction_paid)
