/obj/item/weapon/gun/energy/temperature
	...
	Topic(href, href_list)
		...
		if(href_list["temp"])
			...
			if(amount > 0)
				src.current_temperature = min(600, src.current_temperature+amount)
			else
	...
	process()
		switch(temperature)
			if(0 to 100) charge_cost = 5000
			if(100 to 150) charge_cost = 1000
			if(150 to 250) charge_cost = 500
			if(251 to 300) charge_cost = 100
			if(301 to 400) charge_cost = 500
			if(401 to 500) charge_cost = 1000
			if(501 to 550) charge_cost = 2500
			if(501 to 600) charge_cost = 5000
