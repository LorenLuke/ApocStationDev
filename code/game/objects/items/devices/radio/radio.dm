...
/obj/item/proc/hear_radio(var/obj/item/device/radio/R, var/msg, var/channel)
...
/obj/item/device/radio/talk_into(mob/living/M as mob, message, channel)
	...
	if(GLOBAL_RADIO_TYPE == 1) // NEW RADIO SYSTEMS: By Doohl
	  ...
	  /* ###### Intercoms and station-bounced radios ###### */
		...
		//objects can hear now, yay!
		var/list/obj_heard_masked = list() // masked name or no real name
		var/list/obj_heard_normal = list() // normal message
		var/list/obj_heard_voice = list() // voice message
		var/list/obj_heard_garbled = list() // garbled message
		...
		if (length(heard_masked) || length(heard_normal) || length(heard_voice) || length(heard_garbled))
			...
			if (length(heard_masked))
				...
				for (var/obj/item/R in obj_heard_masked)
					R.hear_talk(src, quotedmsg)
			...
			if (length(heard_normal))
				...
				for (var/mob/R in heard_normal)
					if(istype(R, /mob/living/silicon/ai))
						...
						R.show_message(rendered, 2)
				for (var/obj/item/R in obj_heard_normal)
					R.hear_talk(src, quotedmsg)
			...
			if (length(heard_voice))
				...
				for (var/mob/R in heard_voice)
					if(istype(R, /mob/living/silicon/ai))
						...
						R.show_message(rendered, 2)
				for (var/obj/item/R in obj_heard_voice)
					R.hear_talk(src, quotedmsg)
			...
			if (length(heard_garbled))
				...
				for (var/mob/R in heard_voice)
					if(istype(R, /mob/living/silicon/ai))
						...
						R.show_message(rendered, 2)
				for (var/obj/item/R in obj_heard_garbled)
					R.hear_talk(src, quotedmsg)
