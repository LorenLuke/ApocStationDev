 ...
 **/
	...
	proc/EquipRank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0)
		...
		var/datum/money_account/M = create_account(H.real_name, job.start_pay, job.hour_pay, null)
		if(H.mind)
			var/remembered_info = ""
			...
			remembered_info += "<b>You will make</b> $[job.hour_pay*payout_interval/3600]<b> every [round(payout_interval/60)] minutes with this occupation.</b><br>"
		...
		if(H.mind && job.head_position)
			...
			if (job == "Captain")
				remembered_info += "<b>The station's account number is:</b> #[department_account.account_number]<br>"
				remembered_info += "<b>The station's account pin is:</b> [department_account.remote_access_pin]<br>"
				remembered_info += "<b>The station's account funds are:</b> $[department_account.money]<br>"
	...
	proc/spawnId(var/mob/living/carbon/human/H, rank, title)
		...
		if(job)
			if(job.title == "Cyborg")
				...
				C.secondary_account_numbers = list()
				...
				for (var/i = 1, i<=accounts_access.len, i++)
					var/access_check = accounts_access[i]
					if(access_check in C.access)
						var/department_name=station_departments[i]
						var/datum/money_account/department_account = department_accounts[department_name]
						C.secondary_account_numbers += department_account.account_number
						if (job == "Captain")
							C.secondary_account_numbers += station_account.account_number
		else
