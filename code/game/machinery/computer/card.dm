	...
	if (mode) // accessing crew manifest
		...
		if (authenticated && modify)
			var/carddesc = {"<script type="text/javascript">
								...
								function markSalaryGreen(){
									var nameField = document.getElementById('salaryfield');
									nameField.style.backgroundColor = "#DDFFDD";
								}
								function markSalaryRed(){
									var nameField = document.getElementById('salaryfield');
									nameField.style.backgroundColor = "#FFDDDD";
								}
								function showAll(){
			...
			var/datum/money_account/D = get_account(modify.associated_account_number)
			var/salary = 0
			if(modify.associated_account_number == 0)
				salary = 0
			else
				salary = D.hour_pay

//			D.hour_pay
			carddesc += "<form name='salarynum' action='?src=\ref[src]' method='get'>"
			carddesc += "<input type='hidden' name='src' value='\ref[src]'>"
			carddesc += "<input type='hidden' name='choice' value='salary'>"
			carddesc += "<b>Hourly Salary:</b> $<input type='text' id='salaryfield' name='salary' value='[salary]' style='width:250px; background-color:white;' onchange='markSalaryRed()'>"
			carddesc += "<input type='submit' value='Change hourly wage' onclick='markSalaryGreen()'>"
			carddesc += "</form>"
	...
	switch(href_list["choice"])
		...
		if("access")
			if(href_list["allowed"])
				if(authenticated)
					...
					var/list/accounts_access = list(access_commandaccount, access_medicalaccount, access_engineeringaccount, access_scienceaccount, access_securityaccount, access_cargoaccount, access_civilianaccount)
					var/isaccount = 0
					if(access_type in accounts_access)
						isaccount = 1
					if(access_type in (istype(src,/obj/machinery/computer/card/centcom)?get_all_centcom_access() : get_all_accesses()))
						modify.access -= access_type
						if(isaccount)
							var/index = accounts_access.Find(access_type)
							var/department_name=station_departments[index]
							var/datum/money_account/department_account = department_accounts[department_name]
							modify.secondary_account_numbers -= department_account.account_number
						if(access_allowed == 1)
							modify.access += access_type
							if(isaccount)
								var/index = accounts_access.Find(access_type)
		...
		if ("salary")
			if (authenticated)
				...
				//var/t1 = input(usr, "What name?", "ID computer", null)  as text
				if ((authenticated && modify == t2 && (in_range(src, usr) || (istype(usr, /mob/living/silicon))) && istype(loc, /turf)))
					var/account_num = modify.associated_account_number
					var/new_salary = text2num(href_list["salary"])
					var/datum/money_account/indexed_account = get_account(account_num)
					if(new_salary >= 0)
						indexed_account.hour_pay = new_salary
					else
						src.visible_message("<span class='notice'>[src] buzzes rudely.</span>")
